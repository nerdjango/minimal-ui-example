...
document.addEventListener("get-contract-value", async (evt: any) => {
    if (
        await isSelectedAddress(
            evt.detail as string,
            selectedSigner,
            "Error getting contract value. Selected signer is not the same."
        )
    ) {
...
document.addEventListener("toggle-contract-value", async (evt: any) => {
    if (
        await isSelectedAddress(
            evt.detail as string,
            selectedSigner,
            "Error changing contract value. Selected signer is not the same."
        )
    ) {
...
window.addEventListener("load", async () => {
    try {
        ...
        if (testRpcUrl) {
            ...
            console.log(
                `Provider ready in ${(Date.now() - now) / 1000} seconds`
            );
            now = Date.now();
            ...
            console.log(
                `EVM nonce fetched in ${(Date.now() - now) / 1000} seconds`
            );
        }

        extension.reefSigner.subscribeSelectedSigner(
            async (sig: ReefSignerResponse) => {
                ...
                try {
                    ...
                    if (
                        sig.status ===
                        ReefSignerStatus.SELECTED_NO_VM_CONNECTION
                    ) {
                        throw new Error(
                            "Connect/bind selected account to Reef EVM."
                        );
                    }
                    if (sig.data) {
                        console.log(
                            "signer connected to mainnet =",
                            await isMainnet(sig.data)
                        );
                    }
...
async function getContractValue(sig) {
    try {
        ...
        document.dispatchEvent(
            new CustomEvent("contract-value", { detail: ctrRes })
        );
    } catch (e) {
        document.dispatchEvent(
            new CustomEvent("contract-value", { detail: e.message })
        );
    }
}
